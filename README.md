...
- 用户授权的本地代码工作区、文件浏览、源码搜索和按行读取工具
- 必须逐次人工批准的文件写入、内容预览与 SHA-256 并发保护
...
搜索源码和按行读取 UTF-8 文本。Agent 可以提议创建或覆盖文件，但客户端会先显示目标路径与
内容预览，只有用户明确批准后后端才会写入。覆盖现有文件时还会校验读取时的 SHA-256，避免
批准期间发生的并发修改被静默覆盖；Shell 和 Git 写操作尚未开放。
...
加密，配置文件不保存明文，也不会把密钥传入对话或写入项目目录。保存设置会重启本地 Agent，
并自动重新注册已打开的工作区。关闭“允许申请文件写入”后，后端不会向模型注册 `write_file`
工具；读取仍受已授权工作区和路径规范化规则限制。浏览器开发模式继续通过 `.env` 配置密钥。
...
第一阶段使用 `knowledge/` 下版本化的 Markdown 或纯文本作为单 Agent 知识源。Agent 可以通过
内置工具检索相关段落、按文档 ID 读取正文，并在回答中使用结果返回的文件和行号引用。当前检索
为本地关键词与短语排序，不需要 Embedding 服务或向量数据库。
...
启动前后端后打开 <http://127.0.0.1:3000/knowledge>，或点击聊天侧栏的“知识管理”，即可新建、
导入、编辑、发布和归档文档，并用自然语言问题测试实际检索命中。保存使用文档 revision 做并发
保护；如果文档在编辑期间被其他操作修改，界面会要求重新加载而不是静默覆盖。
